...
NOTE: to run code from irb (from the lib folder) the requires in the classes need to have prepended `./` to work,
rspec from the root folder would work without problem

This solutions uses the Aho–Corasick algorithm to match multiple patterns. The original implementation was taken from 
...
* False positives and negatives are not supported at the moment as it requires a full matching of the invader.
  But it is possible to extend this easily to consider a partial match based on how many lines of the invader appear
  in the right order in the radar output.
  ...
  Aditionally it would be possible to add both of this improvements to consider false positives and false negatives
